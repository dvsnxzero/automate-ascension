    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto bottom-nav-safe md:pb-8">
      ...
      <section className="card p-6 mb-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button className="bg-accent-bg text-accent border border-accent/30 px-4 py-2.5 rounded-xl text-sm font-semibold flex flex-col items-center justify-center min-h-[64px] leading-tight">
            <span>Paper Trading</span>
            <span className="text-[10px] font-normal opacity-70 mt-0.5">&nbsp;</span>
          </button>
          <button className="bg-surface border border-border text-muted px-4 py-2.5 rounded-xl text-sm font-semibold cursor-not-allowed opacity-40 flex flex-col items-center justify-center min-h-[64px] leading-tight">
            <span>Live Trading</span>
            <span className="text-[10px] font-normal mt-0.5">(Phase 5)</span>
          </button>
        </div>
        <p className="text-xs text-muted mt-3">
      ...
      {/* Default Order Settings — grid-rows-[auto_1fr] keeps inputs on one
          baseline even when one label wraps to two lines and the other doesn't. */}
      <section className="card p-6 mb-4">
        <h2 className="text-base font-bold mb-4">Default Order Settings</h2>
        <div className="grid grid-cols-2 gap-4 grid-rows-[auto_1fr] items-start">
          <label className="text-xs text-muted font-medium leading-tight self-end pb-2 row-start-1 col-start-1">
            Max Position Size<br className="hidden sm:inline" /> (% of account)
          </label>
          <label className="text-xs text-muted font-medium leading-tight self-end pb-2 row-start-1 col-start-2">
            Default Stop<br className="hidden sm:inline" /> Loss %
          </label>
          <input
            type="number"
            defaultValue={5}
            className="row-start-2 col-start-1 bg-surface-light border border-border rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:border-accent/50 transition-colors text-theme-text"
          />
          <input
            type="number"
            defaultValue={5}
            className="row-start-2 col-start-2 bg-surface-light border border-border rounded-xl px-4 py-2.5 text-sm w-full focus:outline-none focus:border-accent/50 transition-colors text-theme-text"
          />
        </div>
      </section>
